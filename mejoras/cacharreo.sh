...
URL_BASE='https://administracionelectronica.navarra.es/EmpleoIntermediacion/'
URL_LISTA='listadodeofertas'
HTML_TMP_NAME="html-tmp$(date +%d%m_%H%M)"
JOBFILE="new-jobs.txt"

curl --url "$URL_BASE$URL_LISTA" \
...
if cmp -s last-id.txt current-id.txt; then
  ...
  echo -e "\nSin novedades. Última actualización: $(date +%d/%m/%Y-%H:%M:%S)\n" >> "$JOBFILE"
  ...
  until cmp -s last-id.txt current-id.txt; do
    ...
    href=$(pup ".miniresumen2:nth-child($i) a[commandname=titulo] attr{href}" < $HTML_TMP_NAME)
    printf '\n%s%s\n' "$URL_BASE" "$href" >> $JOBFILE
